...
class DBQuery_SplitterTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testIsIdentifier_WithoutAlias_AsAlias()
    {
    ...
    {
        $this->assertEquals('UPDATE phpunit_test SET description=NULL', DBQuery_Splitter::parse('UPDATE phpunit_test SET description=?', array(null)));
    }
    
    public function testParse_Integer()
    {
        $this->assertEquals('SELECT * FROM phpunit_test WHERE status=10', DBQuery_Splitter::parse("SELECT * FROM phpunit_test WHERE status=?", array(10)));
    }

    public function testParse_Float()
    {
    	$this->assertEquals('SELECT * FROM phpunit_test WHERE status=33.7', DBQuery_Splitter::parse("SELECT * FROM phpunit_test WHERE status=?", array(33.7)));
    }
    
    public function testParse_Boolean()
    {
    	$this->assertEquals('SELECT * FROM phpunit_test WHERE status=TRUE AND disabled=FALSE', DBQuery_Splitter::parse("SELECT * FROM phpunit_test WHERE status=? AND disabled=?", array(true, false)));
    }

    public function testParse_String()
    {
        $this->assertEquals('SELECT id, "test" AS `desc` FROM phpunit_test WHERE status="ACTIVE"', DBQuery_Splitter::parse('SELECT id, ? AS `desc` FROM phpunit_test WHERE status=?', array('test', 'ACTIVE')));
    }

    public function testParse_String_Confuse()
    {
        $this->assertEquals('SELECT id, "?" AS `desc ?`, \'?\' AS x FROM phpunit_test WHERE status="ACTIVE"', DBQuery_Splitter::parse('SELECT id, "?" AS `desc ?`, \'?\' AS x FROM phpunit_test WHERE status=?', array('ACTIVE', 'not me', 'not me', 'not me')));
    }
    
    public function testParse_String_Quote()
    {
    	$this->assertEquals('SELECT * FROM phpunit_test WHERE description="This is a \\"test\\""', DBQuery_Splitter::parse('SELECT * FROM phpunit_test WHERE description=?', array('This is a "test"')));
    }
    
    public function testParse_String_Multiline()
    {
    	$this->assertEquals('SELECT * FROM phpunit_test WHERE description="This is a \\"test\\"\\nWith another line"', DBQuery_Splitter::parse('SELECT * FROM phpunit_test WHERE description=?', array('This is a "test"' . "\n" . 'With another line')));
    }

    public function testParse_Array()
    {
        $this->assertEquals('SELECT * FROM phpunit_test WHERE description IN ("test", 10, FALSE, "another test")', DBQuery_Splitter::parse('SELECT * FROM phpunit_test WHERE description IN ?', array(array("test", 10, FALSE, "another test"))));
    }
	
    public function testParse_Named()
    {
        $this->assertEquals('SELECT id, "test" AS `desc` FROM phpunit_test WHERE status="ACTIVE"', DBQuery_Splitter::parse('SELECT id, :desc AS `desc` FROM phpunit_test WHERE status=:status', array('desc'=>'test', 'status'=>'ACTIVE')));
    }
    
    public function testParse_Like()
    {
        $this->assertEquals('SELECT * FROM phpunit_test WHERE description LIKE "%foo%"', DBQuery_Splitter::parse('SELECT * FROM phpunit_test WHERE description LIKE %?%', array('foo')));
    }

    public function testParse_Like_Named()
    {
        $this->assertEquals('SELECT * FROM phpunit_test WHERE description LIKE "%foo%"', DBQuery_Splitter::parse('SELECT * FROM phpunit_test WHERE description LIKE %:desc%', array('desc'=>'foo')));
    }
